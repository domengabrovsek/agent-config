...
2. **Spawn experts**: based on the issue domain, spawn the matching expert teammates (see `rules/agent-routing.md`). Their guardrails apply throughout the fix. `(review-time: see section note)`
...
9. **Verify**: run `npm run verify:fast` when package.json declares it. Otherwise run the typecheck (`npx tsc --noEmit`), lint, and full test suite. `/mr` runs the full gate once `(hook)`
